	...
	* the units given ID */
	if((char)data[0]=='*'){
		...
		if(len==3){	
			...
			struct ble_event *ble_event = new_ble_event(len-2); // len-2 = 1 corresponds to number of bits of data.
			...
			data_array[0] = (uint8_t)data[len-1]; // len-1 = 2 corresponds to last index
