...
void ble_write_thread(void)
{
	...
	for (;;) {
		...
		if(buf->len<19){ //Uferdig funksjon, kommer uansett ikke til å bruke konsollen på det ferdige produktet
			char data_send[buf->len+2];
			for(int i=buf->len+1; i>1; i--){
				data_send[i]=buf->data[i-2];
			}
			data_send[0] = '*';
			data_send[1] = id;
			if (bt_nus_send(NULL, data_send, buf->len+2)) {
